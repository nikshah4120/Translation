...
fs.readdirSync(mergeFolder).forEach((mergeFile)=>{
     ...
     jsonfile.writeFileSync(path.resolve(translatedFolder,translatedFile),{...translatedLabels,...untranslatedLabels},{spaces:1}); //Merging and upadating files.
     sortJson.overwrite(path.resolve(translatedFolder,translatedFile),{ignoreCase:true});    //Sorting         
     fs.unlink(path.resolve(mergeFolder,mergeFile),(err)=>{if(err) throw err;});  //Removing (i.e t_es_ES.json ) file 
});
